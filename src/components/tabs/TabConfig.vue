...
async function poll() {
    ...
    if (m !== 0 && m !== lastMtime.value) {
        console.log(
            "[TabConfig] 检测到配置文件 mtime 变化，重新加载:",
            lastMtime.value,
            "->",
            m,
        );
...
async function save() {
    ...
    error.value = "";
    try {
        if (view.value.exists)
            await api.saveDirectoryConfig(props.dirPath, view.value);
        else {
            await api.createDirectoryConfig(props.dirPath, view.value);
            view.value.exists = true;
        }
        lastMtime.value = await api.getConfigMtime(props.dirPath);
        // 刷新目录卡片数据，让上方的目录概述信息（参数名、下次压缩时间等）同步更新
...
async function openExternal() {
    await api.openConfigInEditor(props.dirPath);
}
...
<template>
    <div v-if="view">
        ...
        <div v-if="!view.exists" class="no-config">
            ...
            <n-alert v-if="hasError" type="error" class="err">{{
                error
            }}</n-alert>
        </div>
