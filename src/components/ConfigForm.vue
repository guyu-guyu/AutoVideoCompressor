...
import {
  NCard, NSpace, NCheckbox, NSelect, NInput, NInputNumber, NText, NDynamicInput,
  NGrid, NGi,
} from "naive-ui";
...
function setField<K extends keyof DirConfigView>(key: K, value: DirConfigView[K]) {
...
function updateRename(index: number, key: "pattern" | "replacement", value: string) {
...
function onCustomToggle(checked: boolean) {
  if (checked) {
    ...
    const tmpl = props.templates.find(t => t.name === props.modelValue.params);
    emitNormalized({
      ...props.modelValue,
    ...
    emitNormalized({
      ...props.modelValue,
      params: props.templates.length > 0 ? props.templates[0].name : props.modelValue.params,
  ...
  <div class="form">
    ...
    <n-card size="small" title="压缩参数" :bordered="true">
      <n-space vertical :size="8">
        <n-checkbox :checked="isCustom" @update:checked="onCustomToggle">
          自定义参数
        </n-checkbox>
        <n-space align="center" :size="8">
          ...
          />
          <n-input
            :value="isCustom ? props.modelValue.params : selectedTemplateParams"
            :readonly="!isCustom"
        ...
        <n-text v-if="!isCustom && selectedTemplateParams" depth="3">
    ...
    <n-card size="small" title="白名单 INCLUDE(必需)" :bordered="true">
      <n-dynamic-input
        ...
        :on-create="() => ''"
        placeholder="输入包含规则"
        @update:value="(v: unknown[]) => setField('include', v as string[])"
      />
    </n-card>
    ...
    <n-card size="small" title="黑名单 EXCLUDE" :bordered="true">
      <n-dynamic-input
        ...
        :on-create="() => ''"
        placeholder="输入排除规则"
        @update:value="(v: unknown[]) => setField('exclude', v as string[])"
      />
    </n-card>
      ...
      <n-space vertical :size="4">
        <n-space align="center">
          ...
          <n-input-number
            ...
            placeholder="空=不限制"
            style="width:200px"
            @update:value="(v: number | null) => setField('maxCompressSizeMb', v)"
          />
        </n-space>
        <n-text depth="3">超过此总大小的文件将在下次压缩中被跳过</n-text>
      </n-space>
    </n-card>

    <n-card size="small" title="过滤 FILTERS" :bordered="true">
      <n-grid :cols="2" :x-gap="8" :y-gap="8" item-responsive>
        <n-gi><n-space align="center"><n-text>最小 MB</n-text><n-input-number :value="props.modelValue.minSizeMb" style="width:100%" @update:value="(v: number | null) => setField('minSizeMb', v)" /></n-space></n-gi>
        <n-gi><n-space align="center"><n-text>最大 MB</n-text><n-input-number :value="props.modelValue.maxSizeMb" style="width:100%" @update:value="(v: number | null) => setField('maxSizeMb', v)" /></n-space></n-gi>
        <n-gi><n-space align="center"><n-text>修改 ≥</n-text><n-input :value="props.modelValue.mtimeAfter" placeholder="YYYY-MM-DD" @update:value="(v: string) => setField('mtimeAfter', v)" /></n-space></n-gi>
        <n-gi><n-space align="center"><n-text>修改 ≤</n-text><n-input :value="props.modelValue.mtimeBefore" placeholder="YYYY-MM-DD" @update:value="(v: string) => setField('mtimeBefore', v)" /></n-space></n-gi>
        <n-gi><n-space align="center"><n-text>创建 ≥</n-text><n-input :value="props.modelValue.ctimeAfter" placeholder="YYYY-MM-DD" @update:value="(v: string) => setField('ctimeAfter', v)" /></n-space></n-gi>
        <n-gi><n-space align="center"><n-text>创建 ≤</n-text><n-input :value="props.modelValue.ctimeBefore" placeholder="YYYY-MM-DD" @update:value="(v: string) => setField('ctimeBefore', v)" /></n-space></n-gi>
      </n-grid>
    </n-card>
    ...
    <n-card size="small" title="重命名规则" :bordered="true">
      <n-dynamic-input
        ...
        @update:value="(v: unknown[]) => setField('renameRules', v as DirConfigView['renameRules'])"
      >
        <template #default="{ value, index }: { value: DirConfigView['renameRules'][number]; index: number }">
          <div class="rename-row">
            <n-input :value="value.pattern" placeholder="pattern" @update:value="(v: string) => updateRename(index, 'pattern', v)" />
            <n-text>→</n-text>
            <n-input :value="value.replacement" placeholder="replacement" @update:value="(v: string) => updateRename(index, 'replacement', v)" />
          </div>
        </template>
      </n-dynamic-input>
    </n-card>
...
.rename-row { display:flex; gap:8px; align-items:center; width:100%; }
