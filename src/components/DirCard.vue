...
async function compressNow(e: Event) {
    ...
    try {
        await api.compressDirectoryNow(props.card.path);
    } catch (err) {
        console.error("[DirCard] 压缩失败:", props.card.path, err);
        window.$message?.error(String(err));
    }
}
async function stopIt(e: Event) {
    ...
    try {
        ...
        console.error("[DirCard] 停止失败:", props.card.path, err);
        window.$message?.error(String(err));
    }
}
async function removeIt(e: Event) {
    ...
    // 注意：Naive UI 的 dialog.warning() 返回的是对话框响应对象，不是 Promise，
    // 不能 await 后据其真假判断。必须在 onPositiveClick 回调里执行删除，
    // 点击「取消」(onNegativeClick) 不执行任何操作。
    window.$dialog?.warning({
        ...
        onPositiveClick: async () => {
            ...
            try {
                ...
                console.error("[DirCard] 删除失败:", props.card.path, err);
                window.$message?.error(String(err));
            }
        },
    });
    ...
    >
        <n-space vertical :size="6">
            <div class="row1">
                ...
                <n-tag v-else :type="badgeType[card.badge]" :bordered="false">
                    {{
                        card.badge === "valid"
                            ? "● 有效"
                            : card.badge === "unscheduled"
                              ? "● 有效(未排程)"
                              : card.badge === "invalid"
                                ? "● 无效(无配置文件)"
                                : card.badge === "config_error"
                                  ? "● 配置错误"
                                  : card.badge === "overlap"
                                    ? "● 重叠"
                                    : card.badge
                    }}
                    <template v-if="card.badgeDetail"
                        >({{ card.badgeDetail }})</template
                    >
                </n-tag>
                <n-switch
                    :value="card.enabled"
            ...
            <n-text depth="2" class="row2">
                ...
                <template v-if="card.paramsName">
                    · 参数 {{ card.paramsName }}</template
                >
            </n-text>
            ...
            <n-text v-if="card.cycleRiskCount > 0" class="warn"
                >⚠ {{ card.cycleRiskCount }} 个循环风险</n-text
            >

            <div class="row3">
                <n-text depth="3"
                    >上次: {{ card.lastRunTime || "—" }}
                    {{ card.lastRunResult }}</n-text
                >
                <n-text depth="3" class="next"
                    >下次: {{ card.nextRunTime }}</n-text
                >
            </div>
        </n-space>
    </n-card>
